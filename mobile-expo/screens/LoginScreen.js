...
const LoginScreen = ({ navigation }) => {
    ...
    React.useEffect(() => {
        const loadTranslations = async () => {
            ...
            setT({
                welcome: getTranslationSync('welcome', lang),
                signIn: getTranslationSync('signIn', lang),
                customer: getTranslationSync('customer', lang),
                vendor: getTranslationSync('vendor', lang),
                phone: getTranslationSync('phone', lang),
                email: getTranslationSync('email', lang),
                login: getTranslationSync('login', lang),
                sendOtp: getTranslationSync('sendOtp', lang),
                enterOtp: getTranslationSync('enterOtp', lang),
                verifyLogin: getTranslationSync('verifyLogin', lang),
                changeNumber: getTranslationSync('changeNumber', lang),
                or: getTranslationSync('or', lang),
                agreeTerms: getTranslationSync('agreeTerms', lang),
                dontHaveAccount: getTranslationSync('dontHaveAccount', lang),
        ...
        >
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.subtitle}>{t.signIn || 'Sign in to continue'}</Text>
                </View>
                ...
                {role === 'user' && (
                    <View style={styles.methodTabs}>
                        <TouchableOpacity onPress={() => setAuthMethod('otp')} style={styles.methodTab}>
                            <Text style={[styles.methodText, authMethod === 'otp' && styles.methodTextActive]}>{t.phone}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setAuthMethod('password')} style={styles.methodTab}>
                            <Text style={[styles.methodText, authMethod === 'password' && styles.methodTextActive]}>{t.email}</Text>
                        </TouchableOpacity>
                    </View>
                )}
                    ...
                    {step === 'input' ? (
                        <>
                            <View style={styles.inputWrapper}>
                                ...
                                <TextInput
                                    style={styles.input}
                                    placeholder={authMethod === 'otp' ? t.phone : t.email}
                            ...
                            <TouchableOpacity style={styles.primaryBtn} onPress={authMethod === 'password' ? handleLogin : sendOtp}>
                                <Text style={styles.primaryBtnText}>
                                    {authMethod === 'password' ? t.login : t.sendOtp}
                                </Text>
                            </TouchableOpacity>
                        </>
                    ) : (
                ...
                <Text style={styles.termsText}>
                    {t.agreeTerms || "By continuing, you agree to our Terms & Privacy Policy"}
                </Text>
